...
type Stats = { totalSales: number; totalCustomers: number; pendingLab: number; readyLab: number; todaySales: number; monthlySales: number; };
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    setStats({ totalSales:ventas.length, totalCustomers:isVend?ventas.length:0, pendingLab:0, readyLab:0, todaySales:todaySalesData.length, monthlySales:monthlySalesData.reduce((a,v)=>a+(Number(v.total)||0),0) });
    if (!isVend) supabase.from('customers').select('id',{count:'exact',head:true}).then(({count})=>setStats(s=>({...s,totalCustomers:count||0})));
    setLoading(false);
  }, [allSalesData, allPaymentsData, selectedBranch, profile]);
    ...
    return (
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[{label:'Mis Ventas Hoy',value:myTodaySales.length.toString(),icon:<ShoppingCart size={18}/>,sub:'registradas hoy'},{label:'Total Facturado Hoy',value:`Gs. ${myTodayTotal.toLocaleString()}`,icon:<TrendingUp size={18}/>,sub:'mis ventas'},{label:'Pendientes de Entrega',value:myPending.length.toString(),icon:<Clock size={18}/>,sub:'en taller o pendiente'}].map((card,i)=>(
              ...
              {myPending.map(v=>{
                ...
                return(<div key={v.id} className="flex items-center gap-3 px-5 py-3.5">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs font-light mt-0.5" style={{color:'rgba(255,255,255,0.35)'}}>{new Date(v.fecha).toLocaleDateString('es-PY',{day:'2-digit',month:'2-digit'})} · {v.sucursalEntrega} · Gs. {Number(v.total).toLocaleString()}</p>
                  </div>
        ...
        <div className="premium-card overflow-hidden">
          ...
          {myTodaySales.length===0?(<div className="empty-state"><div className="empty-state-icon"><ShoppingCart size={22}/></div><p className="text-sm font-light">Aún no registraste ventas hoy</p></div>):(
            <div className="overflow-x-auto"><table className="w-full text-xs">
              ...
              <tbody>{myTodaySales.map(v=>{const sc=statusConfig[v.estadoTrabajo]||statusConfig.pendiente;return(<tr key={v.id} className="table-row"><td className="px-4 py-3 font-mono text-gold">VTA-{v.id}</td><td className="px-4 py-3 text-white font-light">{`${v.cliente.nombre} ${v.cliente.apellido}`.trim()||'—'}</td><td className="px-4 py-3 text-white font-light">Gs. {Number(v.total).toLocaleString()}</td><td className="px-4 py-3"><span className="status-badge" style={{background:`${sc.color}1e`,color:sc.color}}>{sc.icon}{sc.label}</span></td></tr>);})}</tbody>
            </table></div>
          )}
        </div>
      </div>
  ...
  // ── ADMIN ──────────────────────────────────────────────────────────────────
  const ventasFilt  = selectedBranch ? allSalesData.filter(v=>branchMatch(v.sucursalVenta||'')) : allSalesData;
  const recentSales = ventasFilt.slice(0,8).map(v=>({id:String(v.id),sale_number:`VTA-${v.id}`,total:v.total,status:v.estadoTrabajo||'pendiente',customers:{full_name:`${v.cliente.nombre} ${v.cliente.apellido}`.trim()},branches:{name:v.sucursalVenta}}));
  const pendingSales = ventasFilt.filter(v=>v.estadoTrabajo==='pendiente'||v.estadoTrabajo==='en_laboratorio').sort((a,b)=>new Date(a.fecha).getTime()-new Date(b.fecha).getTime()).slice(0,20);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm font-light mt-1" style={{color:'rgba(197,160,89,0.7)'}}>Bienvenido, {profile?.full_name||'Usuario'} · {new Date().toLocaleDateString('es-PY',{weekday:'long',year:'numeric',month:'long',day:'numeric'})}</p>
        </div>
        <div className="relative">
          <button onClick={()=>setBranchDropdown(!branchDropdown)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-light" style={{background:selectedBranch?'rgba(197,160,89,0.12)':'rgba(255,255,255,0.05)',border:`1px solid ${selectedBranch?'rgba(197,160,89,0.35)':'rgba(255,255,255,0.12)'}`,color:selectedBranch?'#C5A059':'rgba(255,255,255,0.6)'}}>
          ...
          {branchDropdown&&(
            <div className="absolute right-0 top-full mt-1 rounded-xl overflow-hidden z-50 min-w-48" style={{background:'rgba(10,9,7,0.97)',border:'1px solid rgba(197,160,89,0.22)',boxShadow:'0 8px 32px rgba(0,0,0,0.6)'}}>
              <button onClick={()=>{setSelectedBranch('');setBranchDropdown(false);}} className="w-full flex items-center gap-2 px-4 py-3 text-sm font-light text-left" style={{background:!selectedBranch?'rgba(197,160,89,0.10)':'transparent',color:!selectedBranch?'#C5A059':'rgba(255,255,255,0.6)',borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
                {!selectedBranch&&<span className="w-1.5 h-1.5 rounded-full shrink-0" style={{background:'#C5A059'}}/>}Todas las Sucursales
              </button>
              {SUCURSALES.map(s=>(
                <button key={s} onClick={()=>{setSelectedBranch(s);setBranchDropdown(false);}} className="w-full flex items-center gap-2 px-4 py-3 text-sm font-light text-left" style={{background:selectedBranch===s?'rgba(197,160,89,0.10)':'transparent',color:selectedBranch===s?'#C5A059':'rgba(255,255,255,0.6)',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={i} className="stat-card p-5">
              <div className="absolute top-0 right-0 w-20 h-20 rounded-full -translate-y-6 translate-x-6 pointer-events-none" style={{background:'radial-gradient(circle,rgba(197,160,89,0.13) 0%,transparent 70%)'}}/>
              <div className="flex items-start justify-between relative">
                <div><p className="section-label">{card.label}</p><p className="text-2xl text-white font-light mt-1.5">{card.value}</p><p className="text-xs mt-1 text-gold-muted">{card.sub}</p></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="premium-card lg:col-span-2 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 soft-border-bottom"><h2 className="text-white text-sm font-light tracking-wider">Ventas Recientes{selectedBranch?` — ${selectedBranch}`:''}</h2><ShoppingCart size={16} className="text-gold gold-glow-sm"/></div>
          {recentSales.length===0?(<div className="empty-state"><div className="empty-state-icon"><ShoppingCart size={22}/></div><p className="text-sm font-light">No hay ventas registradas</p></div>):(
            <div className="overflow-x-auto"><table className="w-full text-xs">
              ...
              <tbody>{recentSales.map(sale=>{const sc=statusConfig[sale.status]||statusConfig.pendiente;return(<tr key={sale.id} className="table-row"><td className="px-4 py-3 font-mono text-gold">{sale.sale_number}</td><td className="px-4 py-3 text-white font-light">{sale.customers?.full_name||'-'}</td><td className="px-4 py-3 font-light" style={{color:'rgba(255,255,255,0.48)'}}>{sale.branches?.name||'-'}</td><td className="px-4 py-3 text-white font-light">Gs. {Number(sale.total).toLocaleString()}</td><td className="px-4 py-3"><span className="status-badge" style={{background:`${sc.color}1e`,color:sc.color}}>{sc.icon}{sc.label}</span></td></tr>);})}</tbody>
            </table></div>
          )}
        </div>
        <div className="premium-card overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 soft-border-bottom"><h2 className="text-white text-sm font-light tracking-wider">Sucursales (mes)</h2><Building2 size={16} className="text-gold gold-glow-sm"/></div>
          <div className="p-4 space-y-3">
            {branchStats.length===0?(<p className="text-xs text-center py-2 font-light" style={{color:'rgba(255,255,255,0.3)'}}>Sin datos</p>):branchStats.map((b,i)=>(
              <div key={i}>
                <div className="flex justify-between text-xs mb-1.5"><span className="text-white font-light">{b.name}</span><span className="text-gold">{b.count} ventas</span></div>
                <div className="h-1.5 rounded-full overflow-hidden" style={{background:'rgba(255,255,255,0.055)'}}><div className="h-full rounded-full" style={{width:`${Math.min(100,(b.count/Math.max(...branchStats.map(x=>x.count),1))*100)}%`,background:'linear-gradient(to right,#E2C070,#C5A059,#8B6914)',boxShadow:'0 0 8px rgba(197,160,89,0.40)'}}/></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="premium-card overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 soft-border-bottom">
          <div><h2 className="text-white text-sm font-light tracking-wider">Pendientes de Entrega{selectedBranch?` — ${selectedBranch}`:''}</h2><p className="text-xs font-light mt-0.5" style={{color:'rgba(255,255,255,0.35)'}}>En Taller o Pendiente</p></div>
          <Clock size={16} className="text-gold gold-glow-sm"/>
        </div>
